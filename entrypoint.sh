...
if [[ "$INPUT_KEY" ]]; then
    ...
    echo "ssh -v -p ${INPUT_PORT} -o StrictHostKeyChecking=no -i tmp_id ${INPUT_USERNAME}@${INPUT_HOST} 'mkdir -p $INPUT_TARGET'"
    ssh -v -p ${INPUT_PORT} -o StrictHostKeyChecking=no -i tmp_id ${INPUT_USERNAME}@${INPUT_HOST} "mkdir -p $INPUT_TARGET"
