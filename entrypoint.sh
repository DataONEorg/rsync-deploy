...
if [[ "$INPUT_KEY" ]]; then
    ...
    #echo "ssh -v -p ${INPUT_PORT} -i /tmp_id ${INPUT_USER}@${INPUT_HOST} 'mkdir -p $INPUT_TARGET'"
    ssh -p ${INPUT_PORT} -i tmp_id ${INPUT_USER}@${INPUT_HOST} "mkdir -p $INPUT_TARGET"
    rsync -rav -e "ssh -i tmp_id -p ${INPUT_PORT}" ${INPUT_SOURCE} $INPUT_USER@$INPUT_HOST:$INPUT_TARGET
